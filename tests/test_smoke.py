...
async def test_the_steelman_argues_back_every_round() -> None:
    ...
    try:
        ...
        # And it names the connection that actually wrote it. A writer that fell
        # through to a backup wrote on a different endpoint and frequently a
        # different model id, so crediting the seat's primary here would be the
        # misattribution backups exist to prevent.
        spare = cfg.Member(id="a/backup", model="m-a-spare", base_url="https://s/v1",
                           api_key="k", label="Alpha")
        real = s.ask_member

        async def via_backup(m, prompt, system=None, docs=None):
            got = await real(m, prompt, system, docs)
            return Answer(got.member, got.text, ok=got.ok, source=spare)

        s.ask_member = via_backup
        fell = await s.ask_all("q", rounds=2, steelman=s.Steelman())
        s.ask_member = answering
        assert "argued on assignment, written by m-a-spare" in fell, fell
